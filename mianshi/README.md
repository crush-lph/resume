# 面试复习总索引

这份 `README` 是 `mianshi/` 目录的全局索引页。  
目标不是重复所有内容，而是回答三个问题：

1. 这份资料里有什么。
2. 某类问题该去哪个文档找。
3. 每个文档的核心价值是什么。

后续我继续整理资料时，会优先维护这里的索引，再把细节写到对应文档里。

## 使用规则

### 面试前 10 分钟

先看：

- [商汤高频题清单.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/商汤高频题清单.md)

### 面试前 30 分钟

按顺序看：

- [商汤高频题清单.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/商汤高频题清单.md)
- [项目复盘口径模板.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/项目复盘口径模板.md)
- [面试问题题库.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/面试问题题库.md)

### 系统复习

按顺序看：

- [knowledge-base/README.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/README.md)
- [算法速成题单.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/算法速成题单.md)
- [14天复习计划.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/14天复习计划.md)
- [14天复习计划答案.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/14天复习计划答案.md)

## 快速导航

### 0. 项目级 Skill

- [.agents/skills/interview-notes/SKILL.md](/Users/lipenghu/Desktop/hulia/resume/.agents/skills/interview-notes/SKILL.md)
  - 核心内容：面经产出项目级 Skill，定义公司面经、考点专题、实操口径、索引维护的统一流程。
  - 适用场景：后续新增面经、补充题目、维护考点专题、整理高频复习资产。

### 1. 商汤专项

- [商汤高频题清单.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/商汤高频题清单.md)
  - 核心内容：从商汤一面、二三四面提炼出的高频题速记清单。
  - 适用场景：面试前速刷、查“这轮最容易考什么”。

- [interviews/商汤科技一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/商汤科技一面.md)
  - 核心内容：闭包、防抖节流、缓存、并发渲染、响应式、Tailwind、SEO、动态元数据、状态管理、Vibe Coding 资产沉淀。
  - 适用场景：基础题 + 项目追问，尤其是 FastMoss 的 SEO / Next.js 相关表达。

- [interviews/商汤科技二三四五面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/商汤科技二三四五面.md)
  - 核心内容：React 闭包陷阱、Promise 顺序、事件循环、Tree Shaking、Next 水合错误、客户端组件服务端执行、token / SSO / 扫码登录、请求去重、keep-alive、部署流程、pnpm、Vibe Coding。
  - 适用场景：中高阶原理题、工程化题、登录鉴权题、浏览器机制题。

### 1.1 AI / Agent 专项

- [interviews/加和科技一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/加和科技一面.md)
  - 核心内容：超长上下文 Agent 前端优化、可观测、录制方案、Agent 指标体系、AI 全栈工程师理解、从 0 到 1 项目设计、Vibe Coding 大型项目落地注意点。
  - 适用场景：AI 前端、对话式 Agent、可观测性和多媒体能力相关面试。

- [interviews/数字100复试.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/数字100复试.md)
  - 核心内容：Cursor / Claude Code / Codex 工具差异、Skill 设计、Skill 确定性、Skill 组合调用、AI 工程思考。
  - 适用场景：AI 工具链、Skill 设计、Agent 工作流、AI 方法论相关面试。

- [interviews/面经输出模板.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/面经输出模板.md)
  - 核心内容：统一的面经沉淀格式，包括关键考点、注意点、可能追问、可背答案。
  - 适用场景：后续新增任何面经时作为标准模板使用。

### 1.2 React / 工程化专项

- [interviews/上海元符号一面问题.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/上海元符号一面问题.md)
  - 核心内容：上海元符号一面录音问题提取，包含负责人经历、Umi 到 Next、AI 仓库学习助手、LangGraph/Agent、Skill、Vibe Coding、SSE、性能优化、权限和支付。
  - 适用场景：AI 全栈 / Agent 工程化方向面试复盘，尤其适合复盘“项目主导能力”和“AI 实操深度”。

- [interviews/社保卡一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/社保卡一面.md)
  - 核心内容：社保卡一面复盘，包含部署缓存、Tree Shaking、Vue 3、Webpack/Vite、Webpack 优化、微前端、TypeScript、接口签名、AI 工具。
  - 适用场景：Vue / 工程化 / 微前端 / TypeScript 基础与项目追问复盘。

- [interviews/才博教育一二三面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/才博教育一二三面.md)
  - 核心内容：JavaScript 基础、React Hooks、倒计时、Taro 与小程序、Undo/Redo、Promise、架构排障、TypeScript、表单设计、AI 代码治理、三面软素质题。
  - 适用场景：综合型前端面试、项目深挖后继续追基础与工程设计的场景。

- [interviews/AfterShip一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/AfterShip一面.md)
  - 核心内容：增长前端、Umi 到 Next.js 重构、SEO ROI、indexed page 未收录原因、关键词优化、留存漏斗、AB 测试推进。
  - 适用场景：增长前端、SEO 增长、重构 ROI、数据驱动优先级相关面试。

- [interviews/健康界一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/健康界一面.md)
  - 核心内容：性能优化、Next.js 部署模式、负载均衡、SDD 规格驱动开发、Vibe Coding Harness Engineering。
  - 适用场景：Next.js 生产部署、前端性能治理、AI 编程工程化、Vibe Coding 质量护栏相关面试。

- [interviews/爱快路由一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/爱快路由一面.md)
  - 核心内容：项目底层难点、Vibe Coding 最佳实践、Zustand 底层、性能优化、移动端适配、React 渲染流程、Diff、优先级、并发渲染、模块数据隔离。
  - 适用场景：React 原理、工程化、状态管理、性能和权限隔离相关面试。

### 2. 项目复盘

- [项目复盘口径模板.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/项目复盘口径模板.md)
  - 核心内容：FastMoss、Oumomo、MossCreator 等项目的 2 分钟介绍模板和复盘口径。
  - 适用场景：自我介绍、项目展开、技术亮点复盘。

- [knowledge-base/07-project-highlights.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/07-project-highlights.md)
  - 核心内容：支付系统策略模式、Vibe Coding 团队落地、从 0 到 1 设计项目架构。
  - 适用场景：项目亮点、架构题、AI 提效题、支付系统设计题。

- [code-examples/payment-strategy.ts](/Users/lipenghu/Desktop/hulia/resume/mianshi/code-examples/payment-strategy.ts)
  - 核心内容：支付系统策略模式示例代码，包含策略接口、注册表、订单轮询、状态归一化。
  - 适用场景：支付系统被追问到“能不能讲讲代码怎么设计”。

### 3. 通用高频题

- [面试问题题库.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/面试问题题库.md)
  - 核心内容：按项目与主题整理的高频追问题单。
  - 适用场景：抽题自测、模拟面试、补题库。

- [react-interview-notes.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/react-interview-notes.md)
  - 核心内容：React 基础与进阶知识点笔记。
  - 适用场景：React 核心原理补基础，例如事件循环、Hooks、渲染机制。

### 4. 系统知识库

- [knowledge-base/README.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/README.md)
  - 核心内容：知识库总目录。
  - 适用场景：不知道该从哪篇开始时，先看这个。

- [knowledge-base/00-knowledge-map.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/00-knowledge-map.md)
  - 核心内容：整体知识地图和优先级。
  - 适用场景：规划复习顺序。

- [knowledge-base/01-react-core.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/01-react-core.md)
  - 核心内容：React 核心知识。
  - 适用场景：Hooks、渲染机制、组件设计、性能优化。

- [knowledge-base/02-ai-frontend.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/02-ai-frontend.md)
  - 核心内容：AI 前端相关知识。
  - 适用场景：SSE、流式渲染、RAG 展示、AI 交互体验。

- [knowledge-base/03-architecture-security.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/03-architecture-security.md)
  - 核心内容：架构、安全、工程化、AI Agent、Prompt Injection、Vite 预构建。
  - 适用场景：中高阶架构题、安全题、AI Agent 相关题。

- [knowledge-base/04-answer-templates.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/04-answer-templates.md)
  - 核心内容：面试回答模板。
  - 适用场景：把零散知识点压成可说出口的话。

- [knowledge-base/05-review-checklist.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/05-review-checklist.md)
  - 核心内容：纠错与审核清单。
  - 适用场景：复盘答案时自查逻辑漏洞、指标漏洞、表达漏洞。

- [knowledge-base/06-ali-ai-frontend-qa.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/06-ali-ai-frontend-qa.md)
  - 核心内容：阿里 AI 前端题与参考答案。
  - 适用场景：AI 前端专项刷题。

### 5. 复习计划与答案

- [算法速成题单.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/算法速成题单.md)
  - 核心内容：7 天速成题单、14 天冲刺题单、题型到题目的最短映射。
  - 适用场景：准备算法专项、临时补算法、按天刷 LeetCode。

- [14天复习计划.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/14天复习计划.md)
  - 核心内容：两周节奏的复习安排。
  - 适用场景：按天推进，控制复习范围。

- [14天复习计划答案.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/14天复习计划答案.md)
  - 核心内容：复习计划对应的参考回答。
  - 适用场景：快速找答案、校对自己的表达。

### 6. 补充资料

- [notes/Webpack 优化.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/notes/Webpack%20优化.md)
  - 核心内容：Webpack 优化笔记。
  - 适用场景：构建性能、包体、loader / plugin 相关问题。

## 主题到文档的映射

### React / JavaScript 原理题

优先看：

- [interviews/才博教育一二三面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/才博教育一二三面.md)
- [interviews/商汤科技二三四五面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/商汤科技二三四五面.md)
- [interviews/商汤科技一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/商汤科技一面.md)
- [interviews/爱快路由一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/爱快路由一面.md)
- [knowledge-base/01-react-core.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/01-react-core.md)

### Next.js / SEO / SSR / Hydration

优先看：

- [interviews/AfterShip一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/AfterShip一面.md)
- [interviews/健康界一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/健康界一面.md)
- [interviews/商汤科技一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/商汤科技一面.md)
- [interviews/商汤科技二三四五面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/商汤科技二三四五面.md)
- [knowledge-base/07-project-highlights.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/07-project-highlights.md)

### 支付 / 权限 / 登录 / 鉴权

优先看：

- [knowledge-base/07-project-highlights.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/07-project-highlights.md)
- [code-examples/payment-strategy.ts](/Users/lipenghu/Desktop/hulia/resume/mianshi/code-examples/payment-strategy.ts)
- [interviews/商汤科技二三四五面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/商汤科技二三四五面.md)
- [interviews/爱快路由一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/爱快路由一面.md)

### AI 前端 / Vibe Coding / Agent

优先看：

- [interviews/上海元符号一面问题.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/上海元符号一面问题.md)
- [interviews/健康界一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/健康界一面.md)
- [knowledge-base/02-ai-frontend.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/02-ai-frontend.md)
- [knowledge-base/03-architecture-security.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/03-architecture-security.md)
- [knowledge-base/07-project-highlights.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/knowledge-base/07-project-highlights.md)
- [interviews/商汤科技一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/商汤科技一面.md)
- [interviews/商汤科技二三四五面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/商汤科技二三四五面.md)
- [interviews/加和科技一面.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/加和科技一面.md)
- [interviews/数字100复试.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/interviews/数字100复试.md)

### 项目 2 分钟介绍 / 项目深挖

优先看：

- [项目复盘口径模板.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/项目复盘口径模板.md)
- [面试问题题库.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/面试问题题库.md)

## 以后检索资料的默认路径

以后我在这个 `mianshi/` 目录里工作时，会按这个顺序找资料：

1. 先看这份 [README.md](/Users/lipenghu/Desktop/hulia/resume/mianshi/README.md) 的主题映射。
2. 再跳到对应的面经、知识库或代码示例。
3. 如果当前资料没有，再补充到正确的细分文档，而不是把所有内容继续堆在一个文件里。

这样可以保证：

- 入口始终统一。
- 同类问题能快速定位。
- 新资料沉淀后不会丢失在目录里。
